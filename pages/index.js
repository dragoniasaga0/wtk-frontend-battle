...
export default function WTKFullUI() {
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto text-sm">
      ...
      <div className="mb-2">
        ห้อง: <strong>{roomId}</strong> | บทบาท: <strong>{role || "???"}</strong> | ขุนพล: <strong>{hero || "???"}</strong>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {hand.map((card, i) => (
            <div
              ...
              className="border p-4 text-center font-semibold cursor-pointer hover:shadow-xl rounded"
            >
              <Image
